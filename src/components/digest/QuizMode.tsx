...
import { Blurb, QuizQuestion } from '@/types';
import { cn } from '@/lib/utils';
import { Brain, Check, X, Loader2, RotateCcw, Lightbulb } from 'lucide-react';

interface QuizModeProps {
  blurbs: Blurb[];
}

export function QuizMode({ blurbs }: QuizModeProps) {
    ...
    return (
      <div className="rounded-xl border-2 border-dashed border-gray-300 p-8 text-center">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Test your recall before heading out. Can you talk about today&apos;s
          sports?
        </p>
        <button
          onClick={startQuiz}
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-8 text-center">
        ...
        <p className="mt-3 text-sm text-gray-500">Generating your quiz...</p>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-8 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          {percentage >= 80
            ? 'You\'re ready to impress at any party.'
            : percentage >= 50
              ? 'Not bad — review the ones you missed and you\'re golden.'
              : 'Might want to re-read your cheat sheet before heading out.'}
        </p>
        <button
          onClick={resetQuiz}
